## Result: 4:1 — the regression arrived, the win landed (§8 worked)

**Final: Switzerland 4:1 Bosnia (1st half 0:0).** Goals: **Manzambi 74′, Vargas 84′, Manzambi 90′, Mahmić 90+3′ (Bosnia), Jaka 97′ (penalty).** ⚠️ Red card Muharemović (Bosnia, 80′). 5 goals total, BTTS — yes. It was 0:0 until 73' amid total Swiss pressure — super-subs Manzambi+Vargas and the red card opened the floodgates.

**Settlement:**
- ✅ **AI — Switzerland win @1.62 (🟢): +1,860.** Exactly the thesis: 3.24 xG in MD1 → finishing regression. We took the PURE outcome, not the −1 handicap — and it paid (4:1 would've covered the handicap too). §8 in our favour.
- ✅ **Pasha — Switzerland + Over 1.5 @1.80: +2,400.** Win + 5 goals.
- ⏳ Systems: Switzerland 2:0 (AI CS) / 2:1 (Pasha CS) legs — missed (4:1); Switzerland win+O1.5 in Pasha's 3/4 system — ✓.

**Lesson:** confirmation — when a favourite massively underperforms its xG while dominating, the pure win is safer than a handicap or correct score. Caveat: it was 0:0 until 74' and the red card accelerated it — the bet 'hung' in-running. Patience with the robust outcome pays.

---

## Switzerland — Bosnia and Herzegovina · 18 Jun, 22:00 MSK · SoFi Stadium, Los Angeles

**Switzerland generated 3.24 xG in MD1 (26 shots) but scored only once. Expecting positive regression in finishing against Bosnia's bus.**

## Key points
- Switzerland: 26 shots (10 on target) vs Qatar, but drew 1:1. Expecting regression to the mean.
- Bosnia: absorbed 69% possession vs Canada (1:1), goal from a set piece. Deep block.
- Climate: ETFE canopy, 22°C — perfect conditions.
- Models give Switzerland ~61% win probability.

## AI bets
| Bet | Tier | Odds |
|---|---|---|
| Switzerland Win | 🟢 | 1.62 |

**Switzerland Win 🟢** — §8 lesson: pure outcome safer than a handicap. We expect Switzerland to break the bus (3.24 xG in MD1 = finishing regression), but a blowout isn't guaranteed (−1 is dangerous, Bosnia grind 1:1 draws). Odds approximate (European line ~1.62, US ~1.55); clears the 1.6 floor.

## Pasha's bets
- 🟢 **Switzerland win + Over 1.5 @1.80** + a leg of his 3/4 singles system.
- Predicted score (2/4 CS system leg): **2:1**.

**Duel:** both back Switzerland. Pasha strengthens it with Over 1.5 (believes in 2+ Swiss goals), AI takes the pure outcome — a difference in bet form, not thesis.
