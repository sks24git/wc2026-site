## Result: 1:1 — DR Congo snatch a historic point, the handicap lands

**Final: Portugal 1:1 DR Congo (1st half 1:1).** Goals: **João Neves 6′** (header, Neto cross) — **Yoane Wissa 45+5′** (header, Masuaku delivery, DR Congo's first World Cup goal in 52 years). No second-half goals; **Cancelo's goal disallowed on 55′** (VAR, offside). xG: Portugal 0.64 / DR Congo 0.82 — DR Congo were the sharper side despite Portugal's total possession (724 passes). Ronaldo played 90′ without scoring, Leão came on 72′. Yellows: Bernardo 13′, Mbemba 32′, Semedo 88′. No penalties or red cards.

**Settlement:**
- ✅ **AI — DR Congo +1.5 @1.95 (🟢): WIN.** DR Congo didn't lose — the handicap covers the draw. The §8.4 calibration worked: a positional favourite without Rúben Dias against an elite defence (8 clean sheets in 13) = the bus held, no rout. **+2,850 ₽.**
- ❌ **Pasha — 2nd half Over 1.5 @2.07 (🟢): LOSE.** Both goals came in the first half, zero in the second. The "DR Congo score in the 2nd half" thesis didn't land — they equalised late in the 1st half instead. **−3,000 ₽.**
- ⏳ Systems (AI CS 2/4, Pasha CS 2/4, Pasha 2/3 singles) — this match's legs missed (Portugal 1:0 / 2:1 / 2H Over 1.5 — all off), but the systems stay alive into the next matches.
- **Head-to-head of the match: AI beat Pasha** (+2,850 vs −3,000) — a rare one in our favour.

**Lesson:** against an organised bus with two strikers, "a goal in the 2nd half" is weaker than the underdog handicap: the bus can score its one goal at any time (DR Congo did it late in the 1st half). The +1.5 on the correct favourite type (positional, missing its defensive leader) confirmed the Spain/Iran pattern. BTTS Yes would also have landed (Pasha himself regretted not taking BTTS for the price — his instinct was right, his market pick wasn't).

---

## Portugal — DR Congo · 17 Jun, 20:00 MSK · NRG Stadium, Houston

**A positional favourite against an elite defence: Portugal besiege the DR Congo bus without Rúben Dias (injury) inside a climate-controlled dome — value on DR Congo +1.5.**

## Key points

- 🏟 **Retractable roof, full air conditioning** — NRG Stadium, Houston. The Texas heat outside is not a factor: neutral conditions, even pitch. Portugal's technical edge is amplified. Referee: **Abdulrahman Al-Jassim (Qatar)** — experienced (2019 Club WC final, 2022 WC third-place play-off), 3 red cards in 14 matches this season (21% of games).
- **Portugal (Roberto Martínez, 4-2-3-1):** FIFA No. 5. 7 wins in the last 10, Over 2.5 in 9 of the last 13, 71.3% possession, 3.33 goals/match. Martínez leaving after the tournament — maximum motivation to start strong.
  - ⚠️ **Rúben Dias — OUT** (injury in the Nigeria friendly, officially confirmed). Replacement: **Tomás Araújo** (Benfica, only his 2nd cap) — weakened central defence.
  - Ronaldo **PLAYS** (suspended ban already served). Bruno Fernandes — record season (9+21 in PL). Leão likely bench; Pedro Neto or Félix in the starting XI.
- **DR Congo (4-2-3-1 / 5-3-2):** first World Cup since 1974 (as Zaire). CAF qualifying: 7W-1D-2L, 15 goals scored / 6 conceded. AFCON 2025: 0.5 conceded/match (3rd best). Compact low block + counter-attacks.
  - ⚠️ **Yoane Wissa — PLAYS** (recovered from ACL rupture). Pace, Premier League pedigree, the main attacking threat. Aaron Wan-Bissaka (West Ham), Axel Tuanzebe — defensive standard above average for CAF.
  - ⚠️ **Rocky Bushiri — EXCLUDED** (injury). A loss for the centre of their defence.
  - Bakambu (21 international goals) — start vs bench status uncertain.

## Total and trends

- **xG projection:** ~1.8-2.0 (Portugal) + ~0.7-0.9 (DR Congo) = combined xG **2.5-2.9**
- **Models:** Opta — Portugal 73.5% / draw ~18% / DR Congo 8.5%. Kalshi 77%. Dimers: most likely scoreline **2:0** to Portugal. 56% probability of 3+ goals, 66% of Under 3.5 (i.e. 2-3 goals is the most probable range).
- **Market line:** Portugal win ~1.30 (implied 77-78%); Over 2.5 ~1.73 (implied 59%); BTTS Yes ~2.50-2.60 (implied ~38%).
- **H2H:** all-time first meeting. Ronaldo has scored in 3 of his 4 World Cup matches against CAF nations.
- **Portugal trends:** Over 2.5 in 9/13 matches; BTTS Yes in 8/13 matches; only 5 clean sheets in the last 20 matches.
- **DR Congo trends:** failed to score in only 2 of their last 18 matches; World Cup qualifying average 3.0 goals/match.

## Model vs market (⚡ value)

| Market | Implied % | Model | Edge |
|---|---|---|---|
| Portugal win | 77-78% | 73.5% | ~4 pp (no value) |
| Asian handicap −1.5 Portugal | ~52% | ~48% | ~4 pp (weak) |
| Over 2.5 | ~59% | 56% | −3 pp (no value) |
| BTTS Yes | ~38% | **42-45%** | ⚡ **+4-7 pp — value** |
| Ronaldo anytime scorer | ~60% | ~46% | −14 pp (market overpriced) |

**Key takeaway:** the market is not pricing in the absence of Dias and Araújo's defensive vulnerability combined with DR Congo's counter-attacking threat through Wissa. BTTS Yes is cheaper than it should be. Ronaldo anytime is a sentiment-driven overpay.

## AI bets

| Bet | Tier | Odds | Rationale |
|---|---|---|---|
| 🟢 **DR Congo +1.5** | Green | 1.95 | §8.4 calibration: positional favourite vs elite defence (8 clean sheets in 13 qualifying matches, 0.3 conceded/match). No Dias — Araújo starts just his 2nd World Cup game. Opta 23% DR Congo+draw — the Spain-not-Norway pattern. Covers 0:0, DR Congo win, 1:1 and even 2:1 DR Congo. |

**System TS leg (red, inside 2/4 system):** Portugal 1:0 @6.5

**We don't take:**
- Portugal win (-350) — no value, implied matches model
- Ronaldo anytime (-140/-154) — sentiment-driven market overpay (−14 pp against us)
- Asian handicap −1.5 Portugal — weak edge
- BTTS Yes (~2.50) — structurally justified as value, but the DR Congo +1.5 handicap is the primary play this game

