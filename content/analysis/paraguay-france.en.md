## Result: Paraguay 0:1 France (0:0)

**Our engine's modal score was 0:1 — bang on. For an hour and a half France stalled against Paraguay's reference block in the Philadelphia furnace (nearly 50°C pitchside), but on ~70' Doué won a penalty via VAR, Mbappé buried it — and one goal was enough. None of the three-goal show the market wanted: the heat, the bus, no Tchouaméni and Deschamps's economy mode did exactly what we'd priced in. France into the quarters — where Morocco await.**

**Settlement. AI — two of two.** ✓ Only France to score @1.75 (France scored, Paraguay blanked) and ✓ France team total under 2.5 @1.90 (exactly one goal) both landed cleanly. A direct monetisation of the thesis that the market had over-extrapolated France's streak, and against an organised block in the heat they'd scrape the minimum. **Pasha:** France −2 handicap @2.03 missed (a one-goal win); his correct-score acca had already died on the first leg.

**Lesson.** Fading the linear streak extrapolation worked: France's five straight 3+ goal games were built against open or reshuffled defences, while the only genuine block on their route (Senegal in the group) they chiselled at until the 66th — and here it repeated almost to the minute. When the market extends a run in a straight line but the structure of the game (a bus opponent + heat + a lost pivot) screams otherwise, the under on the team total is the honest call, not the "show".

---

## Paraguay — France · 05.07, 00:00 MSK · Lincoln Financial Field, Philadelphia

**The best defensive block of qualifying against the tournament's best attack — in a furnace, under a storm threat, and without Tchouaméni. The market expects a goal show; our engine sees a game France win with two.**

## Key points

- ⚠️ **Tchouaméni is out** — a thigh injury in Friday's training (L'Équipe/RMC, widely confirmed); Koné is expected in the pivot. Part of the line was priced before that news.
- **Extreme heat**: kickoff at the peak of the afternoon blaze in an open-air stadium (35°C+, humid, thunderstorms possible — France's game with Iraq at this very venue was already suspended for two hours).
- **Paraguay are the reference block**: they held Germany to 0.42 xG of their own and 1.48 against across 120 minutes; keeper Gill dragged them through the shootout. Alderete misses out, Diego Gómez returns from suspension.
- **France the conveyor belt**: the first nation in World Cup history to score 3+ in five straight matches; Mbappé has scored in every game of the tournament, Olise has 5 assists (one short of Pelé's record). But the streak was built against open or reshuffled defences: against the only organised block on their schedule (Senegal), France chiselled away until the 66th minute.
- **The referee is Tantashev (Uzbekistan)**, the most permissive whistle of the tournament: a card and a half per game and zero penalties — he personally mutes the cards and penalty markets.
- Paraguay's attack is a metronome at ~0.5 xG per game against anyone; both of their open-play tournament goals came from set pieces, in the air.

## Totals and trends

- The engine's modal score is **0-1, with 0-2 in reserve**. France break blocks after the hour: they have scored in the second half of all four matches.
- The market sits at "exactly three goals" — an extrapolation of France's streak. Our projection is lower: heat, a block, a missing pivot, and Deschamps' economy mode once the tie is settled.
- Corners: France have won 4+ in every game of the tournament (exactly 4 even against Iraq's deep bus), while Paraguay managed 0–1 of their own in their 90-minute matches.
- The cards line has already been crushed to 2.5 by the bookmaker — the market has agreed with peacemaker Tantashev on its own, so there is no value left in the under. We pass, honestly.

## Bets

**AI:**
- 🟢 **Only France to score @1.75** — the headline call, a direct wrapper of 0-1/0-2: France score, Paraguay's metronome stays silent.
- 🟢 **France team total under 2.5 @1.90** — fading the "3+" streak: against the only true block on their schedule France produced their worst attacking profile, and now add the heat and no Tchouaméni.
- 🔴 Correct score 0-1 @7.50 — inside the day's red accumulator with 1-1 in Houston.
- _France corners pulled: on Fonbet the line starts at 5.5 (base 6.5), the market has already priced the siege of a block — no value left at our threshold (same as with cards)._
- After lineups: if Olise starts, we look at "Olise to assist" @~2.50 (every Mbappé goal at this tournament has come through him, plus he takes every set piece).

**Pasha:** awaiting his bets.
