## Belgium — Senegal · 01.07, 23:00 MSK · Lumen Field, Seattle

**Belgium are the slate's shakiest favourite: a 1-1 draw with Egypt (via an own goal), a 0-0 with Iran down to ten men, and only one game where they came alive — a 5-1 rout of New Zealand once Lukaku finally got on. Opta gives them just 45.6% to win. Senegal aren't in form either (losses to France and Norway), but they nicked goals in both defeats and thumped Iraq 5-0 when they needed the goal difference. Seattle in summer is mild — weather is a non-factor for either side.**

## Key points

- 🏟 **Lumen Field, open-air.** Local kickoff is around 1 pm Seattle time — mild, ~17-18°C, cloudy, dry. Not hot, not cold — neutral conditions that help neither side.
- **Line-ups (predicted).** Belgium (4-2-3-1, Garcia): Courtois — Castagne, Mechele/N'Goy, Theate, De Cuyper — Tielemans, Onana/Vanaken — Trossard, De Bruyne, Doku — De Ketelaere (false nine). ⚠️ **Lukaku is expected on the bench again** — the same issue behind two slow Belgian starts (he only came alive as a sub against New Zealand). Senegal (4-2-3-1): likely **Diaw in goal** (⚠️ Mendy injured his knee back in the group stage's third round, status uncertain), Koulibaly captains, Diatta/Nyahate/Gueye/Camara in midfield and defence, Sarr/I. Ndiaye/Mané/Jackson up front.
- **Form.** Belgium won Group G purely on goal difference (1-1 Egypt — an own goal on 66'; 0-0 Iran, red card for N'Goy on 66'; 5-1 New Zealand — Trossard brace, De Bruyne, Lukaku, all after he came on) — **two matches without a goal from open play** before routing a clear underdog in round three. Senegal finished third in Group I (0-0 at half then a 1-3 loss to France, 2-3 to Norway — conceding first and opening up both times, then a business-like 5-0 over Iraq for goal difference) — they advanced as one of the best third-placed sides.
- ⚡ **Context find:** by the tournament's own numbers, Senegal created MORE clear-cut chances against France and Norway than Belgium did against Iran and Egypt — a rare case where the nominal underdog has been the more active side.

## Total and trends

- **Line (Fonbet):** 1 2.20 / X 3.30 / 2 3.65 — the flattest line of the slate; the price on the "favourite" is unusually generous. Models disagree sharply: Opta gives Belgium just **45.6%**, the MARCA supercomputer 56.9%. Double chance (draw or Senegal) @1.67.
- Match total 2.5: over @2.00 / under @1.82 — a coin flip. BTTS: yes @1.75 / no @2.10 (implied ~57% yes).
- Both sides are slow starters: Belgium have zero goals from open play across their first two rounds, Senegal were 0-0 at half in both of their defeats.

## Extended markets (halves, shots, corners, penalties)

- **Halves.** 1st-half result: Belgium 2.75 / Draw 2.20 / Senegal 3.85 — Belgium aren't even the clear favourite for the first half alone. 1st-half total under 1.5 @1.38 (implied ~72%) — the market is confident in a tight start. Both teams to score in the 1st half — "no" @1.17 (implied ~85%) — almost rules out a goal exchange before the break, matching both teams' profile.
- **Corners.** The corners-count market is surprisingly close — Belgium 1.87 / Senegal 2.25(!) — the market isn't confident the "favourite" dominates corners, which tracks with recent real numbers (~7.7/game for Belgium vs ~10.6/game for Senegal). Total corners sits between the 8.5 line (over @1.59 / under @2.22) and 9.5 (over @2.10 / under @1.65).
- **Shots on target.** Match total sits between the 7.5 line (over @1.64 / under @2.12) and 8.5 (over @2.12 / under @1.64) — moderate volume, consistent with a slow-burn, non-explosive script. First shot on target of the match — slightly favours Belgium (1.64 vs 2.12 for Senegal).
- **Penalties.** A scored penalty in the match @3.50 (implied ~28.6%) — a market baseline, nothing specific to this pairing.
- Fouls and yellow cards — deliberately skipped.

## Bets

**AI:**
- 🟢 **Double chance Senegal (draw or win) @1.67** — Belgium haven't structurally earned the right to be a clear favourite (Opta 45.6%, two goalless-from-open-play matches before routing a clear underdog); with xG at parity, the rule that a big backing of the favourite needs a ≥0.7 xG gap directly rules out backing Belgium outright. This is insurance via double chance, not an attack on Senegal to win outright. Counter-test: if De Bruyne finally finds partners without Lukaku starting and Belgium win clean — that's the exact scenario that already happened once, but with Lukaku in the side, who by the forecast isn't starting today.
- 🟡 **Accumulator:** Senegal double chance @1.67 + England team shots over 15.5 (England — DR Congo match) @1.70 = **2.84**.
- 🔴 **Correct-score system position (2/3):** Belgium **1-1** Senegal @6.00.

**Pasha:** 🟢 **2nd half — Over 1.5 @2.06** — both sides looked shaky in the group, but the numbers are clear: of 22 goals in six matches for this pair, 18 came in the second half (6 for Belgium, 12 for Senegal). He doesn't buy an open, fast-starting knockout game: "BTTS is basically screaming at me, but I'll take it through the second-half total instead." He agrees with the machine that Senegal go through — just not sure whether in regulation or extra time.
- 🔴 **Three-fold accumulator:** Senegal double chance @1.71 + England's most productive half (England — DR Congo) — 2nd @2.23 + BTTS yes USA — Bosnia @1.91 = **7.28**.
