## USA — Bosnia and Herzegovina · 02.07, 03:00 MSK · Levi's Stadium, Santa Clara

**USA are at home and in good order — they won Group D (4-1 Paraguay, 2-0 Australia), then calmly handed a dead rubber to Turkey with reserves. But the defence leaks: only 1 clean sheet in 5 games this year, and a debutant, Freese, in goal. Bosnia advanced as one of the best third-placed teams (4 points) and know how to score through Džeko and Kolašinac at set pieces even with a weaker squad. Santa Clara in the evening isn't hot — weather is neutral for both sides.**

## Key points

- 🏟 **Levi's Stadium, open-air.** Kickoff is in the evening local time (~5 pm Pacific), moderate temperature — heat isn't a factor.
- **Line-ups (predicted).** USA (4-2-3-1, Pochettino): Freese — Freeman, Richards, Ream (captain), Robinson — Adams, McKennie — Dest, Tillman, Pulisic — Balogun. ⚠️ **Pulisic is returning from a minor calf issue** — status to confirm before kickoff. Bosnia (4-4-2, Barbarez): Vasilj — Memić, Katić, Muharemović, Kolašinac — Bajraktarević, Šunjić, Bašić, Alajbegović — Džeko (captain), Demirović. Džeko (40) is yet to score at this tournament but remains the main aerial threat.
- **Form.** USA won Group D (4-1 Paraguay — a Balogun brace; 2-0 Australia — both goals in the first half), then deliberately handed Turkey a 2-3 win with reserves and zero motivation — expected rotation, not a red flag. Bosnia finished third in their group (1-1 Canada, 1-4 Switzerland, 3-1 Qatar) — they advanced on 4 points as one of the best third-placed sides.
- ⚡ **USA's weak spot:** just 1 clean sheet in 5 games this year; Freese is a debutant starter in goal. Bosnia's threat isn't combination play — it's the air: their goal against Canada came from a corner (Kolašinac's flick to Lukić), and Switzerland didn't break them until the 74th minute despite total home pressure. Bosnia can sit on a low scoreline for long stretches and strike from set pieces.

## Total and trends

- **Line (Fonbet):** 1 1.40 / X 4.80 / 2 9.00 — USA are big favourites, but not extreme ones. USA's individual total: over 1.5 @1.52 / under @2.55, over 2 @1.90 / under @1.90 (dead even). Match total 2.5: over @1.75 / under @2.10 — a slight lean to the over.
- BTTS: yes @2.03 / no @1.80 — the market barely leans "no" (implied ~55.6%), but this is exactly where USA's real defensive weakness (1 clean sheet in 5) meeting Bosnia's proven set-piece threat makes "yes" a reasonable fade of the consensus.

## Extended markets (halves, shots, corners, penalties)

- **Halves.** 1st-half result: USA 1.82 / Draw 2.45 / Bosnia 7.90. 1st-half total under 1.5 @1.50 — a moderately tight start is expected, though not as strongly as the slate's other two matches. Both teams to score in the 1st half — "no" @1.15 (implied ~87%) — a goal exchange before the break is almost ruled out.
- **Corners.** USA are clear favourites in corners: result market 1.23 / draw 11.50 / Bosnia 4.70(!) — near-total corner dominance is expected. Total 9.5 (over @1.94 / under @1.77) — the market leans slightly to the under.
- **Shots.** Match shots total sits between the 21.5 and 23.5 lines, centred on 22.5 (split evenly, 1.85/1.85); a USA shots handicap of −5.5 to −6.5 reflects territorial dominance. Shots on target sit around the 7.5 line (over @1.70 / under @2.03); first shot on target of the match favours USA @1.45 over Bosnia @2.55.
- **Penalties.** A scored penalty in the match @3.40 (implied ~29.4%) — again a market baseline, no specific signal.
- Fouls and yellow cards — deliberately skipped.

## Bets

**AI:**
- 🟢 **Both teams to score — yes @2.03** — a fade of the market's mild lean to "no": USA's genuine defensive weakness (only 1 clean sheet in 5) meets Bosnia's proven aerial set-piece threat (a goal from a dead ball in almost every game where they haven't completely fallen apart). Counter-test: USA fully neutralise Bosnia's tall defenders at crosses and win clean, as they did against Australia (2-0, both goals in the first half).
- 🟡 **Accumulator:** England — DR Congo match total under 2.5 @1.83 + USA individual total over 2 @1.90 = **3.48** — England-DRC's low-scoring script plus confidence in USA's output at home.
- 🔴 **Correct-score system position (2/3):** USA **2-1** Bosnia @8.50.
- 🔴 **Three-leg accumulator of the day's green picks:** DR Congo +1.5 @1.90 × Senegal double chance @1.67 × USA-Bosnia BTTS yes @2.03 = **6.44** — three independent theses from three different matches in one ticket.

**Pasha:** waiting on his picks — will add once he sends his real ones.
