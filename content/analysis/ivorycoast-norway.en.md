## Result: Côte d'Ivoire — Norway 1:2 (0:1)

Norway ground it out with a late goal and reached the World Cup knockout phase for the first time ever. Nusa opened the scoring on 39' (Ødegaard assist), Diallo levelled on 74' (Pépé assist), but on 86' Haaland tapped in Berg's cut-back to win it. Côte d'Ivoire, with no genuine striker, again couldn't finish the job: one goal and done — the story of their tournament. xG 1.36 — 2.02: Norway deserved it. Next up — Brazil (5 July, MetLife).

**Settlement.** AI: 🟢 both teams to score — yes @1.70 — ✓ landed (1:2). Pasha: 🟢 over 2.5 @1.90 — ✓ (three goals); 🔴 comeback win by either team @6.30 — ✗ (Norway led from 39', nobody came from behind to win), −1000 (two reds of 500). Multi-match bets still live: the AI correct-score system nailed this match EXACTLY (1:2) — one more score of two needed (France 2:1 / Mexico 1:0); the AI accumulator and Pasha's accumulator are alive (their CI-Norway legs landed); Pasha's score system with 2:2 missed — both remaining scores must now hit.

**Lesson.** "Comeback win by either side" is a pretty flier, but it needs an actual come-from-behind win; a match where the favourite leads from the first goal and only once lets it level doesn't cash it. The base profile (BTTS + over) did hit: Norway's leaky defence and CI's grit produced exactly the script we expected.

---

## Côte d'Ivoire — Norway · 30.06, 20:00 MSK · AT&T Stadium, Dallas

**A disciplined block with no out-and-out striker against Haaland's vertical thrust. Norway are the better side but leak to everyone, while Côte d'Ivoire reliably cling to their one goal. The model sees a tight game (xG almost level), not a rout — expect goals at both ends and a nervy finish.**

## Key points
- **Roof closed, air-conditioned** — climate neutralised, no "African endurance" bonus. Even tempo.
- **Both finished second, on 6 points.** Côte d'Ivoire (Group E): 1-0 Ecuador (Diallo 90'), 1-2 Germany (led and broke late at 90+), 2-0 Curaçao. Norway (Group I): 4-1 Iraq and 3-2 Senegal (Haaland a brace in each), 1-4 France — but Haaland and Ødegaard were rested there, so discount that hammering.
- **Line-ups (predicted).** Norway (4-3-3): Nyland — ... Ødegaard and Haaland **return fresh to the XI**; RB Ryerson a fitness doubt. Côte d'Ivoire (4-3-3): Y. Fofana — Singo(?), Agbadou, Ndicka, Konan — Sangaré, Kessié, S. Fofana — Adingra, **Pépé as the centre-forward**, A. Diallo. ⚠️ No clean striker — Haller out of the XI; Singo's status unclear. Player markets only after official sheets.
- **H2H:** never met — the first match in history.
- **Referee:** not officially appointed at the time of writing.
- **Team profiles.** Côte d'Ivoire: can run a game plan and endure, but without a finisher can't put it away, and the legs fade after ~70' (how they broke against Germany). Norway: the Haaland–Ødegaard vertical works, but they concede in almost every game — BTTS-yes is their chronic profile.

## Total and trends
- Model (footballmeister): Norway ~46% to win in regulation, projected 2-1, **xG 1.35 (CIV) — 1.58 (NOR)** — a tiny gap. This is NOT a one-team game.
- Market: Norway favourite but not heavily (result ~2.15, to qualify 1.57). The line bakes in a closed game and a real chance of extra time.
- xG sum ~2.9 + Norway's leaky defence + CIV scoring ~1 a game even against top sides → **BTTS-yes on profile**. But if the CIV block smothers the game without a chance of their own — a low score and a shootout.

## Bets
_(odds — Fonbet's live line, 30.06)_

**AI:**
- 🟢 **Both teams to score — yes @1.70** — Norway concede to everyone (Iraq, Senegal), and CIV cling to their goal (Kessié vs Germany, Diallo vs Ecuador). A direct read of both profiles.
- 🔴 In the 2/3 correct-score system — **Norway 2-1** (the model's modal score).
- Counter-test: a straight Norway win and the −1.5 handicap I pass — the model gives only ~46%, the xG are level, CIV are the best block at their level. "Norway don't lose in regulation" is fragile in a closed game.

**Pasha:**
- 🟢 **Over 2.5 @1.90** — for Pasha the Africans are simply stronger, Norway's defence leaks (see Senegal), and CIV score in every game; he doesn't buy a low 1-1, more like 2-2.
- 🟢 In the day's push-protected accumulator — **over 2.0**.
- 🔴 **Comeback win by either team @6.30** — a flier: one bet covering a comeback for either side, expecting an end-to-end swing.
- 🔴 In the 2/3 correct-score system — score **2-2**.
