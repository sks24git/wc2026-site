...
import { pl, stakeOf } from '@/lib/calc';
...
export function buildCards() {
  ...
  const cards = Object.values(map).map((c) => {
    ...
    return {
      ...
      pending,
      status,
      staked: c.list.reduce((s, b) => s + stakeOf(b), 0), // всего поставлено за день (все ставки)
      atStake: c.list.filter((b) => b.status === 'pending').reduce((s, b) => s + stakeOf(b), 0), // сколько сейчас в игре, ₽
    };
  });
